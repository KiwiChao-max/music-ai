...
from app.worker_limits import MemoryPressureError, enforce_memory_limit
...
def _run_pipeline(
    ...
    Each step calls ``_report`` once with its starting progress
    percentage; the actual work runs *before* the next step's
    ``_report``, so the UI sees the previous value until the new one
    lands.
    """
