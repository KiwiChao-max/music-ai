...
export function Skeleton({
  ...
  return (
    <div
      aria-hidden="true"
      className={`${width} ${height} animate-pulse rounded bg-slate-200 ${className}`}
    />
  );
}
...
export function ErrorState({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 break-words rounded bg-rose-100 px-2 py-1 font-mono text-xs text-rose-900">
        {message}
      </p>
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-3 inline-flex items-center justify-center rounded-md bg-rose-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-rose-700"
        >
          Retry
        </button>
      )}
